/////   START BREEFING-ROOM    /////
  ...
  function countSearchResults() {
    if (!$('#search-list').length || !$('.search__result-count').length) {
      return;
    }
    const counter = $('.br-latest__col').length;
    $('.search__result-count').text(counter)
    const resultsElement = $('.search__wrap label').animate({ opacity: 1 }, 200);
  }
...
document.addEventListener('DOMContentLoaded', async function () {
 ...
 initSlider();   
  ...
  $('.br-nav__brand').click(function () {
    const userType = localStorage.getItem('clientType')
    if (userType === 'agency') {
      window.location.href = '/agency-breefingroom';
    }
    if (userType === 'brand') {
