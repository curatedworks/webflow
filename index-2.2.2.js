/////   START BREEFING-ROOM    /////
  ...
  function countSearchResults() {
    if (!$('#search-list').length || !$('.search__count').length) {
      return;
    }
    const counter = $('.br-latest__col').length;
    $('.search__count').text(counter)
    const resultsElement = $('.search__result-count').animate({ opacity: 1 }, 200);
  } 
  /*
  filterByClientType('#filteredArticles .br-latest__col', '.br-card__filter', null)
  filterByClientType('.user-filter .br-latest__col', '.br-card__filter', null)
  ...
  */
  ...
  // Disabling buttons
  const btns = $('#user-type-form .btn');
  btns.addClass('btn--disable');
  $('#utf-email').on('input', function() {
    var val = $(this).val();
    if (val.length) {
      btns.removeClass('btn--disable');
    } else {
      btns.addClass('btn--disable');
    }
  });

  // Showing popup settings
  $("#utf-brand").on('click', function () {
    if ($('#user-type-form').is(':invalid')) {
      return;
    } else {
      window.localStorage.setItem('clientType', 'brand');
      $("input.user-type").val('brand');
      const formId = $(this).closest('form').attr('id');
      $(`#${formId}`).submit();      
    };
  });
  $("#utf-agency").on('click', function () {
    if ($('#user-type-form').is(':invalid')) {
      return;
    } else {
      window.localStorage.setItem('clientType', 'agency');
      $("input.user-type").val('agency');
      const formId = $(this).closest('form').attr('id');
      $(`#${formId}`).submit();      
    };
  });

  $('#user-type-form').on('submit', function () {
    const id = $(this).attr('id');
    const userType = localStorage.getItem('clientType')
    $('.w-form-done, .w-form-fail').hide();
    if ($(`#${id}`)[0].checkValidity()) {
        window.location.href = '/breefingroom';
    }
  });
  ...
  if (!localStorage.getItem('newsletterWasShown')) {
    setTimeout(() => {
      $('.modal-news__trigger').click();
      localStorage.setItem('newsletterWasShown', true);
    }, 60000);
  }
  ...
  $('#email-form, #email-form2').on('submit', function () {
    ...
    if ($(`#${id}`)[0].checkValidity()) {
      ...
      let url = `https://projects.breef.com/public/signup?email=${email}`;
      if (firstName) {
  ...
  $('#email-form4').on('submit', function () {
    const id = $(this).attr('id');
    $('.w-form-done, .w-form-fail').hide();
    let url = `https://projects.breef.com/public/signup`;
    window.open(url);
  });
  ...
  if ($('#newsletterForm').length) {
    ...
    $('#newsletterForm').on('submit', function () {
      if ($('#newsletterForm').is(':invalid')) {
        return;
      } else {
        $('.modal-news__close').click();
  ...
  $('#cta-footer').on('submit', function () {
    if ($('#cta-footer').is(':invalid')) {
      ...
      let url = `https://projects.breef.com/public/signup?type=agency`;
      window.open(url);
    }
  });
  }
  ...
  $('#email-form--agency').on('submit', function () {
    ...
    if ($(`#${id}`)[0].checkValidity()) {
      const email = $(`#${id} input[type="email"]`).val();
      const firstName = $(`#${id} input#firstName`) ? $(`#${id} input#firstName`).val() : '';
      let url = `https://projects.breef.com/public/signup?type=agency&email=${email}`;
      if (firstName) {
        url += `&first_name=${firstName}`;
      }
      window.open(url);
